...
namespace TCH.BackendApi.Configuration
{
    public class ProductConfiguration : IEntityTypeConfiguration<Product>
    {
        public void Configure(EntityTypeBuilder<Product> builder)
        {
            ...
            builder.HasMany(x => x.PromotionProducts).WithOne(g => g.Product).HasForeignKey(y => y.ProductID);
            builder.HasMany(x => x.OrderDetails).WithOne(g => g.Product).HasForeignKey(y => y.ProductID);
            builder.HasMany(x => x.ProductSizes).WithOne(g => g.Product).HasForeignKey(y => y.ProductID);
        }
    }
}
